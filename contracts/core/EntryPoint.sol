...
contract EntryPoint is IEntryPoint, StakeManager, NonceManager, ReentrancyGuard, OpenZeppelin.ERC165 {
    ...
    /**
     * _执行单笔<操作>: this.innerHandleOp -> _handlePostOp
     * Execute a user operation.
     * @param opIndex    - Index into the opInfo array. 索引: 用于错误追踪
    ...
    // 执行<操作>集: _validatePrepayment -> _validateAccountAndPaymasterValidationData -> _executeUserOp *-> _compensate
    ...
    /// @inheritdoc IEntryPoint
    // 执行聚合<操作>集: aggregator.validateSignatures -> _validatePrepayment -> _validateAccountAndPaymasterValidationData -> _executeUserOp *-> _compensate
    ...
    /// @inheritdoc IEntryPoint
    // 模拟<操作>(单笔): _simulationOnlyValidations -> _validatePrepayment -> _intersectTimeRange -> _executeUserOp -> call
    ...
    /**
     * _执行<操作>: <Gas检查> -> call -> _handlePostOp
     * 只能用 external 进行修饰: 其上下文参数是 calldata 类型的(仅看此处还不能清楚为什么)
    ...
    /**
     * 类型转换: <MemoryUserOp> sender + nonce + 5个Gas字段 + paymaster(paymasterAndData的前20字节)
    ...
    /// @inheritdoc IEntryPoint
    // 模拟<验证>: _simulationOnlyValidations -> _validatePrepayment -> _intersectTimeRange
    function simulateValidation(UserOperation calldata userOp) external {
        ...
        ReturnInfo memory returnInfo = ReturnInfo(outOpInfo.preOpGas, outOpInfo.prefund, sigFailed, data.validAfter, data.validUntil, getMemoryBytesFromOffset(outOpInfo.contextOffset));
    ...
    /**
     * 尝试创建合约账户: senderCreator.createSender
    ...
    // 校验(仅模拟): this._validateSenderAndPaymaster
    ...
    /**
     * 校验(仅模拟): sender 必须有合约基础， paymaster 必须为已部署合约
    ...
    /**
     * 检验 sender 账户是否有足够的预付款(优先看当前合约余额): _createSenderIfNeeded -> IAccount(sender).validateUserOp
     ...
     */
    function _validateAccountPrepayment(uint256 opIndex, UserOperation calldata op, UserOpInfo memory opInfo, uint256 requiredPrefund) internal returns (uint256 gasUsedByValidateAccountPrepayment, uint256 validationData){
        unchecked {
            uint256 preGas = gasleft();
            MemoryUserOp memory mUserOp = opInfo.mUserOp;
            ...
            try IAccount(sender).validateUserOp{gas: mUserOp.verificationGasLimit}(op, opInfo.userOpHash, missingAccountFunds) returns (uint256 _validationData) {
    ...
    /**
     * 检验 paymaster 账户是否有足够的预付款: deposits[paymaster] -> IPaymaster(paymaster).validatePaymasterUserOp
     ...
     */
    function _validatePaymasterPrepayment(uint256 opIndex, UserOperation calldata op, UserOpInfo memory opInfo, uint256 requiredPreFund, uint256 gasUsedByValidateAccountPrepayment) internal returns (bytes memory context, uint256 validationData) {
        unchecked {
            MemoryUserOp memory mUserOp = opInfo.mUserOp;
    ...
    /**
     * 校验预付款: _validateAccountPrepayment -> _validateAndUpdateNonce -> _validatePaymasterPrepayment
    ...
    function _validatePrepayment(uint256 opIndex, UserOperation calldata userOp, UserOpInfo memory outOpInfo) private returns (uint256 validationData, uint256 paymasterValidationData){
        ...
        uint256 maxGasValues = mUserOp.preVerificationGas | mUserOp.verificationGasLimit | mUserOp.callGasLimit | userOp.maxFeePerGas | userOp.maxPriorityFeePerGas;
